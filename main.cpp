...
void is_valid_num_round( int num_round )
{
  return 1 <= num_round && num_round <= 10;
}

void round_is_over(RoundState const& r)
{
  return r.city_events && r.rulers_decisions;
}

void is_final_round(RoundState const& r)
{
  return r.num_round == FINAL_ROUND;
}

int32_t get_new_citizen_num(CityState const& city_state, CityEvents const& city_events){
  auto new_citizen_num = city_events.num_citizen_dead / 2 * (5 - city_events.bushels_wheat_per_acre) * city_state.bushels_wheat / 600 + 1;
...
CityEvents gen_new_city_events(int32_t const seed, CityState const& city_state){
  ...
  int32_t bushels_wheat_with_harvested = city_state.bushels_wheat + city_events.bushels_wheat_per_acre * city_state.num_acre;
  city_events.bushels_wheat_eaten_rats = bushels_wheat_with_harvested * percent_bushels_eaten_rats_distrib(gen);

  constexpr int32_t bushels_wheat_per_citizen = 20;
  // Если есть остаток, то человек всё равно погибает с голоду
  int32_t num_citizen_can_be_fed = city_state.bushels_wheat / bushels_wheat_per_citizen;
  city_events.num_citizen_dead = std::max(0, city_state.num_citizen - num_alive_citizen);
  ...
  while(true) {
    ...
    if( n == 1 )
    {
      ...
      while(state.size() < FINAL_ROUND)
      {
        ...
        if(round_is_over(current_round) && !is_final_round){
    ...
    {
      std::<< "Debug: Не реализовано" << std::endl;
      return 0;
    }
  }
  return 0;
