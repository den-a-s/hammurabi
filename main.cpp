...
void print_message_to_ruler(RoundState const &rs)
{
  ...
  int bushels_wheat = cs.bushels_wheat +
...
{
  if (P > 33.0 && L < 7.0)
    return GameEndEnum::Bad;
  if (P > 10.0 && L < 9.0)
    return GameEndEnum::Normal;
  if (P > 3.0 && L < 10.0)
    return GameEndEnum::Normal;
  return GameEndEnum::Great;
}

void print_game_end_message(GameEndEnum gee)
{
  std::string message;
  switch (gee)
  {
  case GameEndEnum::Bad:
    message = "Из-за вашей некомпетентности в управлении,\n"
              "народ устроил бунт, и изгнал вас их города.\n"
              "Теперь вы вынуждены влачить жалкое существование в изгнании";
    break;
  case GameEndEnum::Normal:
    message = "Вы правили железной рукой, подобно Нерону и Ивану Грозному.\n"
              "Народ вздохнул с облегчением, и никто больше не желает видеть вас правителем";
    break;
  case GameEndEnum::Good:
    message = "Вы справились вполне неплохо, у вас,\n"
              "конечно, есть недоброжелатели, но многие хотели\n"
              "бы увидеть вас во главе города снова";
    break;
  case GameEndEnum::Great:
    message = "Фантастика! Карл Великий, Дизраэли и Джефферсон вместе не справились бы лучше!";
    break;
  }
  std::cout << message << std::endl;
}

void run_game(StartGameState cmd)
{
  GameState state = start_game_state(cmd);
  while (state.rounds.size() < FINAL_ROUND)
  {
    auto &current_round = state.rounds.back();
    ...
    current_round.city_events =
        gen_new_city_events(state.seed, current_round.city_state);
    ...
    state.rounds.push_back(get_next_round(current_round));

    save_game(state);

    std::cout << "Желаете продолжить? [y(default)/n] : ";
    std::string answer;
    std::cin >> answer;
    if (answer == "n")
      return;
  ...
  double P = calc_mean_dead_persons(state.rounds);
  double L = calc_num_acres_per_citizen(state.rounds);

  print_game_end_message(is_win(P, L));
}
...
// UTF-8 позволяет кроссплатформенно выводить русские символы в консоль
  ...
  if (n == 1 || n == 2)
  {
    run_game(static_cast<StartGameState>(n));
  }
  else
  {
    std::cout << "Таких пунктов нет" << std::endl;
  }
  return 0;
}
